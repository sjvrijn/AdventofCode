...
for minute in range(1, num_minutes+1):
    ...
    if field_hash in hashes:
        cycle_length = minute - hashes[field_hash]
        print(minute, hashes[field_hash], cycle_length)
        break
    else:
        hashes[field_hash] = minute
...
remaining_minutes = num_minutes - hashes[field_hash]
final_hash_equivalent = (remaining_minutes % cycle_length) + hashes[field_hash]

for field_hash, idx in hashes.items():
    if idx == final_hash_equivalent:
        break

print(field_hash.count(str(int(Landscape.TREES))) * field_hash.count(str(int(Landscape.LUMBERYARD))))

...
num_minutes = 0
spreading = set()
spreading.add(Point(*np.argwhere(maze == Maze.OXYGEN)[0]))
draw_spread = False
verbose = False
...
while spreading:
    ...
    # ... filtered by if they are 'space'
    for coord in spreading:
        for neighbor in directions.values():
            new_coord = coord + neighbor
            if maze[new_coord] == Maze.SPACE:
                to_spread.add(new_coord)

    if not to_spread:
        break

    for coord in to_spread:
        maze[coord] = Maze.OXYGEN
    ...
    if draw_spread:
        plt.imshow(maze)
        plt.title(str(num_minutes))
        plt.savefig(plot_path / f"15b-{num_minutes:03d}.png")
        plt.clf()
